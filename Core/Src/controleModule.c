...
uint8_t readRegister(uint8_t addr) {
        ...
        HAL_StatusTypeDef status = HAL_SPI_TransmitReceive(&hspi2, pCmd, pData, 1, 100);
        /*
        HAL_StatusTypeDef status = HAL_SPI_Transmit(&hspi2, pCmd, cmdSize, SPI_TIMEOUT);
        if ( status )
    	   status = HAL_SPI_Receive(&hspi2, pData, dataSize, SPI_TIMEOUT );*/
        return data;
...
void writeRegister(uint8_t addr, uint8_t val) {
	...
	HAL_StatusTypeDef status = HAL_SPI_Transmit(&hspi2, pWrite, writeSize, SPI_TIMEOUT);
	if( status )
		status = HAL_SPI_Transmit(&hspi2, &val, sizeof(val), SPI_TIMEOUT);
